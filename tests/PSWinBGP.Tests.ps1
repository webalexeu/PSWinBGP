Describe 'PSWinBGP' {
    ...
    It 'Function: Start-WinBGPRoute' {
        Start-WinBGPRoute -ComputerName localhost -Name 'winbgp.contoso.com' | Should -Be "@{Result=API connection timeout}"
    }
    It 'Function: Stop-WinBGPRoute' {
        Stop-WinBGPRoute -ComputerName localhost -Name 'winbgp.contoso.com' | Should -Be "@{Result=API connection timeout}"
    }
